#!/usr/bin/env bash
# Iter-171 regression test pinning the UTF-8 locale invariant guard added at script entry of iter-150 (readable git-log renderer), iter-152 (5-panel commits-health histogram), and iter-153 (pre-commit advisor). Pre-iter-171 these scripts inherited operator LC_ALL — CI runners with LC_ALL=C (common systemd-default misconfiguration) would silently byte-count subject length via bash dollar-hash-var expansion, mis-classifying a CJK subject (15 visible characters, 27 UTF-8 bytes) as 27 chars under C locale and false-triggering 72-char hard-cap warnings. Iter-171 closes the silent-fail-class gap by injecting a case-statement guard at the top of all three scripts that maps empty/unset/C/POSIX to en_US.UTF-8 while respecting any other operator-set UTF-8 locale (en_CA, zh_CN, C.UTF-8, etc.) verbatim. Test asserts (a) each script source contains the guard, (b) guard correctly upgrades C to UTF-8, (c) the canonical iter-153 advisor returns 15 chars for the 15-char CJK probe subject under hostile LC_ALL=C (proving the end-to-end pipeline), (d) iter-150 renderer still functions under LC_ALL=C, (e) guard comment cites Conventional Commits section 5 character-counting semantics rationale.
set -euo pipefail
...
if grep -qF "measured length:        ${ITER171_EXPECTED_VISIBLE_CHAR_COUNT_FOR_PROBE_SUBJECT} chars" <<<"$ITER171_ITER153_ADVISOR_OUTPUT_UNDER_HOSTILE_C_LOCALE"; then
...
ITER171_TOTAL_ASSERTIONS_EVALUATED=$((ITER171_TOTAL_ASSERTIONS_EVALUATED + 1))
if grep -qF "measured length:        ${ITER171_EXPECTED_UTF8_BYTE_COUNT_FOR_PROBE_SUBJECT_USED_AS_NEGATIVE_CONTROL_TO_DETECT_REGRESSION_TO_PRE_ITER171_BYTE_COUNTING} chars" <<<"$ITER171_ITER153_ADVISOR_OUTPUT_UNDER_HOSTILE_C_LOCALE"; then
