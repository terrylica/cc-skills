#!/usr/bin/env bash
# Iter-159 regression test wrapping the empirical end-to-end pre-commit-framework validation harness. Verifies (a) harness script exists + bash-clean + shellcheck-clean, (b) pre-commit binary is available (else skip gracefully), (c) harness runs end-to-end and exits 0 against the current cc-skills HEAD (which is the iter-158-manifest-language-script-bug-fix commit or later). This regression test gates against re-introducing the language:system bug iter-159 caught in iter-158, and catches any future regression in the entry-point's commit-msg-file extraction or iter-153 advisor delegation when invoked through the real pre-commit framework.
set -euo pipefail
