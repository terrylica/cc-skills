#!/usr/bin/env bash
# Iter-119 regression test for the --json output mode flag added to the iter-116 reverse-search CLI. Verifies (1) --json flag is documented in the CLI's usage message; (2) --json on a known consumer emits valid JSON parseable by jq with status=found + markers array containing the lifecycleLayer string field + the full registry entry; (3) --json on a plausible-typo query emits valid JSON with status=not-found + didYouMean array sorted ascending by Levenshtein distance + allRegisteredConsumerSourceFilePaths is null (didYouMean non-null path); (4) --json on a truly-unrelated query emits valid JSON with status=not-found + didYouMean=null + allRegisteredConsumerSourceFilePaths populated; (5) JSON output is emitted to stdout (not stderr) so it can be piped to jq; (6) exit codes are unchanged from iter-116/iter-118 baseline (0 found, 2 not-found, 1 usage error); (7) human-readable mode (no --json flag) is preserved unchanged.
