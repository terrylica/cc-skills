#!/usr/bin/env bash
# Iter-156 regression test pinning the operator-facing commits arc discoverability entry point. Asserts (a) tasks/commits/_default exists + executable + bash-clean + shellcheck-clean, (b) default-dispatcher output mentions all 5 operator tools by name (release:history, commits:health, commits:health --json, commits:advise, commits:advise --json) plus the iter-154 COMMIT_EDITMSG auto-detect mode, (c) iter-150 50/72 convention reminder is present with both thresholds (50 + 72), (d) canonical spec URLs are cited (conventionalcommits.org + cbea.ms), (e) docs/RELEASE.md cross-reference is present, (f) docs/RELEASE.md contains the Conventional-Commits Operator Toolkit Index section.
set -euo pipefail
