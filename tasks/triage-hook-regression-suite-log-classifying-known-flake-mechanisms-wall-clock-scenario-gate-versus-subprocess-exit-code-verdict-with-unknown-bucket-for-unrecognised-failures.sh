#!/usr/bin/env bash
# Triage a repo:test-hooks suite log into the known intermittent-failure mechanisms so a red run is diagnosed from evidence instead of re-run until green. Reads a captured suite log (moon run repo:test-hooks > log 2>&1) and classifies each failing assertion as CLASS A (a wall-clock scenario gate exceeded its cap — load-sensitive), CLASS A? (two harness invocations disagreed and the failing scenario is UNNAMED, so the reduction to CLASS A is INFERRED, not observed), CLASS B (the iter-160 doctor ran and reported a non-healthy verdict — a CRITICAL check returned non-zero, which is fork/exec pressure rather than elapsed time), CLASS C (a genuine mode-dispatch or JSON-leak defect, positively distinguished from CLASS A by the harness reporting that NO scenario failed), or UNKNOWN (matches no known mechanism — deliberately never absorbed into the nearest bucket, so a third mechanism has to announce itself). Prints the margin on every wall-clock failure because a 0.9%-over run and a 300%-over run are different diagnoses and must never render identically.
#
# ═══════════════════════════════════════════════════════════════════════════
#  Provenance
# ═══════════════════════════════════════════════════════════════════════════
