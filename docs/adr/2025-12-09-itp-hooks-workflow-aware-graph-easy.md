...
The `pretooluse-guard.sh` hook blocks writes to markdown files containing ASCII box-drawing characters (≥10 chars) without a `<summary>graph-easy source</summary>` block. This enforcement ensures reproducible diagrams.

However, this creates a **workflow friction** when using graph-easy correctly:

1. Claude runs `graph-easy` via Bash → gets rendered ASCII output
2. Claude uses Write/Edit to add that output to markdown file
3. PreToolUse hook sees box-drawing characters, no source block yet → **BLOCKS**
...
```
 ⏮️ Before: Graph-Easy Output Blocked
...
```
graph { label: "⏮️ Before: Graph-Easy Output Blocked"; flow: south; }
...
```
 ⏭️ After: Workflow-Aware Detection
...
```
graph { label: "⏭️ After: Workflow-Aware Detection"; flow: south; }
[ Claude ] { shape: rounded; }
[ Run graph-easy ]
[ PostToolUse Hook ]
[ Write Flag ]
...
- **Workflow continuity**: Using graph-easy → writing output should not be interrupted
...
1. **Disable blocking entirely** — Too permissive, loses enforcement
2. **Transcript-based detection** — Check if graph-easy appears in recent tool calls (false positives)
3. **Session flag file** — PostToolUse writes flag, PreToolUse checks it
4. **Environment variable** — Pass state via env (doesn't persist across tool calls)
...
```
🏗️ Architecture: Inter-Hook Communication via Session Flags

╭────────────╮     ┌─────────────┐  writes flag   ┌───────────┐
│ Bash Tool  │ ──> │ PostToolUse │ ─────────────> │ State Dir │
╰────────────╯     └─────────────┘                └───────────┘
                                                    ∧
                                                    │
                                                    │
...
```
graph { label: "🏗️ Architecture: Inter-Hook Communication via Session Flags"; flow: east; }
...
- Natural workflow preserved: graph-easy → write works without blocking
...
- **Related ADR**: [Plan File Exemption](/docs/adr/2025-12-09-itp-hooks-plan-file-exemption.md) — Similar exemption pattern
