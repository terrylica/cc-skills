...
| `✓` / `✗401 3d` / `⚠503 3m` | `canary_self_test.last_observed_http_status_code` + `consecutive_failures` × `configured_interval_secs` | Canary outcome glyph + the OFFICIAL last_observed_http_status_code verbatim + humanized failure duration when degraded (AU/QT/CF/UP/IN letter codes retired 2026-06-11 — they re-labeled official statuses). The raw consecutive-failure count is logged to `~/.claude/doorward-state.jsonl` (L2 telemetry), not displayed |
