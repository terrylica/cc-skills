...
test("isTransientOsaError matches retry-worthy AppleEvent failures only", () => {
  expect(isTransientOsaError("execution error: Notes got an error: AppleEvent timed out. (-1712)")).toBe(true);
  expect(isTransientOsaError("execution error: application isn’t running. (-600)")).toBe(true);
  ...
  expect(isTransientOsaError("execution error: Not authorized to send Apple events to Notes. (-1743)")).toBe(false);
  expect(isTransientOsaError("syntax error: Expected end of line (-2741)")).toBe(false);
...
test("contentPresent tolerates reflow and CJK joins but catches missing content", () => {
  expect(contentPresent("Hello world this is a draft", "Hello world this is a draft and footer")).toBe(true);
  // reflowed with different whitespace → still present
  expect(contentPresent("Hello world\nthis is a draft", "Hello   world this\nis a draft")).toBe(true);
  // CJK reflow (no spaces) → still present
  expect(contentPresent("关于本次电汇诈骗\n的取证结论", "关于本次电汇诈骗的取证结论如下")).toBe(true);
  // truncated/empty read-back → caught
  expect(contentPresent("Hello world this is a draft", "")).toBe(false);
  expect(contentPresent("Hello world this is a draft", "(no such draft)")).toBe(false);
...
test("parseRecords splits FS/RS streams and drops empty records", () => {
  const raw = ["F", "iCloud", "Notes", "141"].join(FS) + RS + ["F", "iCloud", "To-Do / Done", "11"].join(FS);
