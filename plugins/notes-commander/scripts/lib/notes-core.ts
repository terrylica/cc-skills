...
export function isTransientOsaError(stderr: string): boolean {
  return /\(-600\)|\(-609\)|\(-1712\)|isn.t running|not running|timed out/i.test(stderr);
}
 ...
 */
export function runOsa(script: string, args: string[], maxAttempts = 3): OsaResult {
  let last: OsaResult = { ok: false, stdout: "", stderr: "osascript did not run", attempts: 0 };
...
export function runOsaOrDie(script: string, args: string[], maxAttempts = 3): string {
