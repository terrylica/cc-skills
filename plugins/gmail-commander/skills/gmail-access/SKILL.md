...
- **2026-07-10 — drafts hard-wrapped at recipient (premature line wrapping).**
  - _Trigger_: sent drafts (Curve-Dental correspondence to the clinic) reflowed to short, fixed-width lines in the recipient's client. Cause: `buildRawMessage` set `Content-Type: text/plain; charset=utf-8` with no `format=flowed`, so long author paragraphs are treated as fixed lines and every client hard-wraps them at its own width.
  - _Fix_: added `toFormatFlowed()` (RFC 3676) + `stuffFlowed()` in `lib/gmail-drafts.ts`; both the plain and multipart/mixed body parts now emit `format=flowed` with 72-col **soft** breaks (trailing space → client REFLOWS) while intentional breaks (list items, `Best,`/name sign-off, blank lines) stay **hard** (no trailing space) and are preserved exactly. Content-Type on both paths is now `text/plain; charset=utf-8; format=flowed`.
  - _Evidence_: unit test shows a long paragraph split into trailing-space soft lines + a final hard line; list/sign-off lines stay hard. Recreated a live draft (msg 19f4d47…) — `read --json` body shows `"… notes are \r"`, `"answered \r"` (trailing-space soft breaks). Rebuilt binary (`bun run build`), tsgo clean. NOTE: the `gmail` binary is compiled — a TS edit needs `bun run build` to take effect; existing drafts made before the fix must be recreated to benefit.
